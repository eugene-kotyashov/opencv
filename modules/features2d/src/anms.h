...
vector<KeyPoint> topN(vector<KeyPoint> keyPoints, int numRetPoints) {
  vector<KeyPoint> kp;
...
vector<KeyPoint> gridFAST(Mat Image, int numRetPoints, int gridRows,
                              int gridCols) {
  vector<KeyPoint> kp;
  Ptr<FeatureDetector> featdetectorK =
      new GridAdaptedFeatureDetector(FeatureDetector::create("FAST"),
...
vector<KeyPoint> brownANMS(vector<KeyPoint> keyPoints,
                               int numRetPoints) {
...
vector<KeyPoint> sdc(vector<KeyPoint> keyPoints, int numRetPoints,
...
template <typename T>
void generatePointCloud(PointCloud<T> &point, vector<KeyPoint> keyPoints) {
...
vector<KeyPoint> kdTree(vector<KeyPoint> keyPoints, int numRetPoints,
                            float tolerance, int cols, int rows) {
...
vector<KeyPoint> rangeTree(vector<KeyPoint> keyPoints, int numRetPoints,
                               float tolerance, int cols, int rows) {
  // several temp expression variables to simplify solution equation
  int exp1 = rows + cols + 2 * numRetPoints;
  long long exp2 =
...
vector<KeyPoint> ssc(vector<KeyPoint> keyPoints, int numRetPoints,
                         float tolerance, int cols, int rows) {
  // several temp expression variables to simplify solution equation
  int exp1 = rows + cols + 2 * numRetPoints;
  long long exp2 =
...
/*
void VisualizeAll(Mat Image, vector<KeyPoint> keyPoints,
                  string figureTitle) {
  Mat resultImg;
  drawKeypoints(Image, keyPoints, resultImg,
                    Scalar(94.0, 206.0, 165.0, 0.0));
  namedWindow(figureTitle, WINDOW_AUTOSIZE);
  imshow(figureTitle, resultImg);
  return;
}
*/
}
